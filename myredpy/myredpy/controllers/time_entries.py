...
class TimeEntries(Controller):
    ...
    def today(self):
        ...
        for p in projects:
            project_hours = sum(
                x.hours for x in self.time_entries(self.user_id(), p.id))
    ...
    def week(self, show_weekends=False):
        ...
        for p in self.app.api.projects():
            entries = self.time_entries(
                self.user_id(), p.id, monday, monday + timedelta(days=5))
            days_to_check = range(0, days_to_show)
            entry_per_day = []
            day = monday
            for d in days_to_check:
                hours = sum(e.hours for e in filter(
                    lambda x: x.spent_on == day, entries))
                entry_per_day.append(hours)
                headers.append('{}'.format(day.strftime('%a-%d.%m')))
                day = day + timedelta(days=1)

            project_entry = [to_multiline_text(
                p.name, omitt_prefix=prefix_to_omitt)]
    ...
    def month(self):
        ...
        data = self.time_entries_by_date_range(self.app.api.projects(), date_range, total_days)
        self.app.render(data, headers="firstrow", tablefmt='fancy_grid')
    ...
    def time_entries_by_date_range(self, projects, date_range, total_days):
        ...
        for p in projects:
           ...
           # set project name as header
            data[0][current_project] = to_multiline_text(p.name, max_line_length=20, max_length=60, omitt_prefix=prefix_to_omitt)
